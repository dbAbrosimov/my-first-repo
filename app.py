...
if mapping_upload is not None:
    ...
    st.sidebar.warning(
        "Файл mapping CSV не загружен — все метрики попадут в 'other'.")
    ...
    for _, elem in etree.iterparse(BytesIO(file_bytes), tag='Record'):
        ...
        if t.endswith('SleepAnalysis'):
            if elem.get('value') != 'HKCategoryValueSleepAnalysisAsleepDeep':
                elem.clear()
                continue
            val = (datetime.fromisoformat(elem.get('endDate')) -
            ...
            records.append({'date': dt_start.date(),
                            'metric': 'Bedtime', 'value': bedtime_val})
...
def analyze_pairs(wide_df, p_thr, min_N):
    ...
    for idx, (X, Y) in enumerate(pairs):
        ...
        if res.pvalue > p_thr:
            ...
            continue
        results.append(
            {'X': X, 'Y': Y, 'r': res.rvalue, 'p': res.pvalue, 'N': N})
        progress_bar.progress((idx + 1) / len(pairs))
    progress_bar.empty()
...
x_type = st.sidebar.selectbox("Ось X", types, index=types.index(
    "HKQuantityTypeIdentifierStepCount") if "HKQuantityTypeIdentifierStepCount" in types else 0)

st.sidebar.markdown("### Метрика Y (ночь)")
y_type = st.sidebar.selectbox("Ось Y", types, index=types.index(
    "HKCategoryTypeIdentifierSleepAnalysis") if "HKCategoryTypeIdentifierSleepAnalysis" in types else 0)

# Выбор периода агрегации
period = st.sidebar.selectbox('Гранулярность', ['День', 'Неделя', 'Месяц'])
# Порог p-value
p_thr = st.sidebar.slider(
    'Порог значимости p-value',
    min_value=0.01,
    max_value=0.1,
    value=0.05,
    step=0.01)
...
if not pairs_df.empty:
    ...
    if drop_same:
        ...
        df2 = df2[df2.apply(lambda row: get_group(row['X']) != get_group(
            row['Y']), axis=1)].reset_index(drop=True)

    # Поиск в таблице
    search = st.text_input('🔍 Поиск метрик (X или Y)', '')
    if search:
        mask = df2['X'].str.contains(
            search, case=False) | df2['Y'].str.contains(
            search, case=False)
    ...
    if st.sidebar.checkbox(
        'Показать топ-5 межгрупповых связей по |r|',
            value=False):
        ...
        st.dataframe(top5[['X', 'Y', 'r', 'p', 'N', 'ΔY',
                     'OptX']], use_container_width=True)
else:
...
with st.expander("Интерактивное описание"):
    ...
    if not pairs_df.empty and 0 <= selected_row_index < len(df2):
        ...
        st.write(
            f"**Оптимальное значение X (где Y >= 95% max):** {row['OptX']}")
        # Plot scatter
        orig_X = row['X_raw']
        orig_Y = row['Y_raw']
        series = wide_df[[orig_X, orig_Y]].dropna()
